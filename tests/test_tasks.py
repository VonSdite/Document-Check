...
class TaskExecutionTest(unittest.TestCase):
    ...
    def test_document_check_sends_full_text_once(self):
        ...
        db.execute(
            ...
            VALUES (
                ...
                ?, 'test-model', 'http://example.test/v1/chat/completions', 30, 20000,
                'running', 0, ?, ?
            )
            """,
    ...
    def test_long_document_check_runs_chunks_and_merges_duplicate_issues(self):
        db = get_db()
        created_at = now_text()
        db.execute(
            """
            ...
            VALUES (
                '127.0.0.1', 'long.pdf', 'long.pdf', 'pdf', 1,
        ...
        with patch("app.tasks.run_check", side_effect=fake_run_check):
            results = _run_check_items_concurrently(
                self.app,
                task,
                [{"code": "typo", "name": "错别字检查", "prompt": "检查错别字"}],
                document_text,
                max_workers=1,
                stream_trace_enabled=False,
            )
        ...
        self.assertTrue(all("long_document_chunk:" in call["document_text"] for call in calls))
        merged = json.loads(results[0]["result"])
        self.assertEqual(len(merged["items"]), 1)
        self.assertIn("长文档", merged["summary"])

    def test_long_document_consistency_verifies_cross_chunk_fact_conflict(self):
        db = get_db()
        created_at = now_text()
        db.execute(
            """
        ...
        def fake_run_check(**kwargs):
            ...
            if "事实提取" in name:
                fact_calls += 1
                value = "220 V" if fact_calls == 1 else "110 V"
                return json.dumps(
                    {
                        "summary": "事实",
                        "items": [
                            {
                                ...
                                "confidence": "high",
                                "category": "设备输入电压",
                                "location": f"分段{fact_calls}",
                                "excerpt": f"额定输入电压为{value}",
                                "description": "额定值",
                                "impact": value,
                                "suggestion": "交流输入模式",
                            }
            ...
            if "跨分段证据复核" in name:
                verified_inputs.append(kwargs["document_text"])
                return json.dumps(
                    {
                        "summary": "确认冲突",
                        "items": [
            ...
            return json.dumps({"summary": "未发现分段内部问题", "items": []}, ensure_ascii=False)

        with patch("app.tasks.run_check", side_effect=fake_run_check):
            results = _run_check_items_concurrently(
                self.app,
                task,
                [{"code": "consistency", "name": "全文一致性检查", "prompt": "检查全文冲突"}],
                document_text,
                max_workers=1,
                stream_trace_enabled=False,
            )

        self.assertGreater(fact_calls, 1)
        self.assertTrue(verified_inputs)
        self.assertIn("220 V", verified_inputs[0])
