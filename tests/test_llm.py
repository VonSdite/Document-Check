...
class LLMResponseParsingTest(unittest.TestCase):
    ...
    def test_document_check_prompt_clamps_issue_output_limit_to_hard_max(self):
        ...
        self.assertIn('"status":"issue|suggestion"', user_content)
        self.assertNotIn('"status":"issue|suggestion|non_issue"', user_content)
        self.assertIn("统一三级判定规则", user_content)
        self.assertIn("满足明确问题条件则输出", user_content)
        self.assertIn("满足待确认建议条件才输出", user_content)
        self.assertIn('不得输出 "non_issue"', user_content)
