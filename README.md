...
所选模型提供商的“单次请求文本上限”用于控制全文请求大小。提取后的文本加文件名超过上限时，普通文档检查会拒绝提交；“一致性检查”也会在合并素材文档和资料文档后做同样的上限校验。

“一致性检查”页面支持上传 1-5 个素材文档和 1-3 个资料文档。资料通常是根据素材文档写作生成的，系统会以素材文档作为依据，调用所选模型检查资料内容是否存在口径不一致、遗漏、偏差或需要人工确认的内容，并输出报告。
