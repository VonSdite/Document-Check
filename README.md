...
- 模型开启思考时，无正文的思考响应达到 32,000 段或 128,000 字符会提前中止，保留防止持续思考的保护；关闭思考时对应限制为 64 段或 256 字符。GLM 5.3 支持纯思考错误后的关闭思考重试。
...
有活动任务时，任务列表和详情页每 10 秒局部更新。列表按运行阶段显示解析文件、等待模型响应、模型思考、模型输出、重试和整理结果；并发检查处于不同阶段时显示“检查中”。详情页在每个检查项标题旁用小字显示当前阶段，检查项结束后移除小字。后台标签页暂停轮询，返回页面时同步状态；详情页保留滚动位置、展开内容和视频播放器，选中文本或编辑报告控件时暂缓替换内容。状态更新失败会显示提示并自动重试。
...
“原始结果”展示模型返回并由系统保存的正文，用于核对整理后的报告条目，不包含模型的思考过程。图片和视频的原始结果可能包含按批次整理的内容。
