...
- 任务执行：后台线程从 SQLite 队列拉取任务，默认全局并发 3、单 IP 并发 1、单任务检查项并发 1，可在管理面调整；长文档会按片段分批送入模型，避免单次请求塞入全文。
...
2. 系统先保存上传文件，再按文档类型提取可检查文本：
   ...
   - `txt`、`md`：按文本文件读取。
   - `html`：去除脚本和样式后提取页面文本。
3. 如果无法提取文本，系统会拒绝提交任务，并删除本次上传文件。文档检查任务的长文本会在后台按片段检查；超出系统分块数量上限的超大文档需要先按章节拆分；一致性检查仍要求合并后的文本不超过所选模型提供商的文本上限。
4. 校验通过后，任务进入队列，后台调度器按全局并发和单 IP 并发限制执行。
5. 执行时会按检查项并发调用模型，并持续写入结果和进度；单任务检查项并发数可在管理面调整。长文档分块时，每个片段都会带有位置线索和输出数量限制，结果按片段汇总到同一个检查项报告中。
6. 任务完成后可在任务列表进入报告页查看结果；完成状态的任务支持导出 HTML 报告。

## 长文档分块规则

普通“文档检查”任务会在后台自动分块，避免把全文一次性送入模型。分块只影响普通文档检查；“一致性检查”需要跨文档对照，仍使用合并后的全文并受模型文本上限约束。

单个片段的目标长度由所选模型提供商的“单次请求文本上限”计算：

```text
片段目标长度 = max(2000, min(12000, 单次请求文本上限 * 0.5))
```

例如模型文本上限为 80000 字时，单片段目标长度为 12000 字；上限为 5000 字时，单片段目标长度为 2500 字。

切分顺序如下：

1. 优先按空行分隔的自然段落切分。
2. 如果全文没有空行段落，则按换行切分。
3. 如果单个段落仍超过片段目标长度，则按中文/英文句末标点切分。
4. 如果单句仍超过片段目标长度，则按固定长度硬切，保证单次请求不会过长。

执行时，每个检查项会逐片段调用模型。片段请求会附带“长文档片段 N/M”和片段开头文本作为位置线索，并要求模型只检查当前片段、不推断其他片段、不输出思考过程；每个片段最多输出 8 条重点问题。最终报告会在同一个检查项下按“片段 N/M”汇总结果。

为了防止超大文档触发过多模型请求，系统最多允许拆成 60 个片段。超过上限时任务会失败并提示先按章节拆分文档后再提交。
