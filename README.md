...
一个基于 Flask + SQLite 的文档智能门禁网站，支持用户上传 docx、pdf、txt、md、html、xlsx、xlsm、xls 文档，并通过用户自行配置的 OpenAI Chat Completions 兼容模型执行单文档规范性、一致性、错别字检查、多文档对照检查、跨语种文档一致性检查和多模态图片检查。
...
2. 系统先保存上传文件，再按文档类型提取可检查文本：
   - `docx`：提取段落和表格文本。
   - `pdf`：使用 PyMuPDF 提取 PDF 页面文本层并保留 pypdf 质量回退，同时基于字符坐标清理视觉上不存在的重叠空格。
   - `txt`、`md`：按文本文件读取。
   - `html`：去除脚本和样式后提取页面文本。
   - `xlsx`、`xlsm`、`xls`：按工作表逐行提取单元格文本。
3. 如果无法提取文本，系统会拒绝提交任务并删除本次上传文件。单文档检查、多文档对照和跨语种检查对中小文档使用全文模式，超过直接请求预算后自动切换为对应的长文档模式，不会仅因合并文本超过模型单次输入上限而拒绝提交。
4. 校验通过后，任务进入队列，后台调度器按全局并发和单用户并发限制执行。
5. 执行时会按检查项并发处理，并持续写入结果和进度；普通 AI 检查项会调用模型，单文档“敏感词检查”会读取本地词表做确定性匹配，不调用模型；图片检查仅支持 PDF，会生成页面截图并提取内嵌图片，按“页面级检查”和“图片资源检查”分组合并调用多模态模型，每批最多 4 张图，并只附带当前页附近的文档文本；单任务检查项并发数可在管理面调整。
6. 任务完成后可在任务列表进入报告页查看结果；完成状态的任务支持导出 HTML 报告。

管理端会缓存每个任务的报告条目统计，仅在报告结果或误报规则变化后重新计算；任务运行期间的自动刷新使用轻量状态接口，只读取当前页任务的状态和进度，不重复加载文档全文或历史报告正文。

单文档长文档模式会按 PDF 页码、章节和段落边界切分文本，并保留少量相邻上下文。错别字、规范性和扩展检查项按分段执行后去重汇总；敏感词检查继续在本地扫描全文；“全文一致性检查”除分段内部检查外，还会为各分段提取参数、要求、条件、安全等级和术语定义等结构化事实，生成跨分段潜在冲突候选，再把候选双方的原文证据交给模型复核。默认直接请求上限不超过 12 万字符，单个长文档分段不超过 4 万字符，并会根据模型提供商配置的文本上限自动缩小。

多文档长文档模式会先保留每个素材文档和资料文档的独立边界，再分别按页码、章节和段落切分。系统使用章节编号、数字、型号、单位、URL、拉丁词和中文短语在本地检索最相关的素材证据，逐个检查资料分段；随后反向以素材分段检索资料证据，补充检查关键要求是否被遗漏。跨语种长文档模式会使用章节编号、数字/型号/单位等硬线索和相对顺序对齐两侧分段，逐组比较，并对未可靠匹配的章节单独执行覆盖复核。两种模式都会增加模型请求次数，并在每批完成后保存中间结果，最终统一去重汇总。
...
“单文档检查”任务会先在本地提取文本，并以 `file: 文件名` 加正文内容的形式交给模型；中小文档直接使用全文，长文档自动分段检查。系统不会把原始文件直接转发给模型。

所选模型提供商的“单次请求文本上限”用于计算全文直传预算和长文档分段/对照批次预算。超过全文直传预算时，单文档、多文档对照和跨语种任务会自动切换为长文档模式；该配置仍应按模型实际上下文能力设置，过小会产生更多批次，过大则可能造成单次请求超出模型能力。

系统设置中的“每次模型回复最多问题条数”会随任务结果保存。新版结构化报告要求模型为每条结果提供严重程度和证据可信度；后端会先合并内容相同、仅位置不同的重复问题，再按严重程度、证据可信度和条目类型排序，最后按该上限展示。原始模型回复仍会完整保留；如果模型遗漏分级字段，系统会为避免误过滤而不执行硬截断，并在报告中提示。

报告条目被判定为“非问题”、选择“不接纳”，且原因为“模型幻觉”“模型误报”或“不适用”时，系统会生成误报忽略候选规则。管理员在系统设置中启用后，规则会在相同任务类型和检查项内按问题描述进行相似匹配；描述相似度达到 56% 的条目会折叠到“已忽略误报”，不参与主报告统计，原始条目仍保留供查看。
...
“多文档对照检查”页面支持上传 1-5 个素材文档和 1-3 个资料文档。资料通常是根据素材文档写作生成的，系统会以素材文档作为依据，调用所选模型检查资料内容是否存在口径不一致、遗漏、偏差或需要人工确认的内容，并输出报告。超长任务采用“资料检索素材证据 + 素材反向覆盖资料”的双向检查，避免简单截断合并全文。多文档对照项可在系统设置中单独维护，支持修改内置提示词、新增扩展检查项、停用、删除和排序；提交任务时会保存所选检查项快照，后续修改提示词不会影响已提交任务。

“跨语种文档一致性检查”页面输入文档A和文档B。中小文档直接结合静态预检和两侧全文检查；超长任务按章节、硬线索和顺序对齐分段，并单独复核未匹配内容，最终汇总缺失、增补、误译、约束强弱变化和关键事实差异。

“图片检查”页面仅支持 PDF。系统会渲染 PDF 页面截图，同时尝试提取内嵌图片并保存到 `instance/extracted_images/`；页面截图用于图文与界面步骤一致性、图表标题可见性、完整性和清晰度等版式类检查，内嵌图片优先用于语种匹配、设备安装与接线、画图规范等资源类检查，缺少首选图片源时会自动回退到另一类图像并在报告中提示人工确认。长文档会优先覆盖含图、含表和内嵌图片所在页，超过页面上限时按段抽样并汇总未覆盖页数。执行时系统按当前页附近文本裁剪上下文，组内多个检查项合并为一次多模态请求，再按结构化小节拆回各检查项结果。内置图片检查项包括图文与界面步骤一致性、图片语种匹配、设备安装与接线、图表标题可见性复核、图片完整性和清晰度、画图规范；历史拆分出的“界面截图与步骤一致性检查”“设备外观与安装图检查”会在默认迁移中停用以减少重复报告。检查项可在系统设置中修改提示词、新增扩展检查项、停用、删除和排序；提交任务时会保存所选检查项快照。图片检查报告会在每个检查项末尾汇总明确问题和需人工确认内容，便于快速浏览。

系统设置中的“检查报告最多保留天数”用于定期清理历史报告，`0` 表示不自动清理；填写正整数后，后台调度器会定期删除超过保留天数的已完成、失败或已取消任务，并同步删除对应上传文件和提取图片，排队中和运行中的任务不会被清理。
