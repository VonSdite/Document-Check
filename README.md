...
- 模型开启思考时，无正文的思考响应达到 64,000 段或 128,000 字符会提前中止，保留防止持续思考的保护；关闭思考时对应限制为 64 段或 256 字符。GLM 5.3 支持纯思考错误后的关闭思考重试。
...
有活动任务时，任务列表和详情页每 10 秒局部更新。列表统一显示“排队、解析、检查、取消中、完成、部分完成、失败、已取消”；等待模型、思考、输出、重试和整理结果统一归为“检查”。详情页在每个检查项标题旁用小字显示当前阶段，检查项结束后移除小字。后台标签页暂停轮询，返回页面时同步状态；详情页保留滚动位置、展开内容和视频播放器，选中文本或编辑报告控件时暂缓替换内容。状态更新失败会显示提示并自动重试。
...
每个模型检查项的内容顶部提供“模型输出内容”折叠区，分开显示服务返回的思考和正文。展开后约每秒读取新增文本并追加，折叠或切到后台时暂停读取；任务状态和报告仍每 10 秒更新。各次请求、重试和多媒体批次分别展示，图片与视频标注合并请求。页面保留输出区的展开状态和滚动位置，位于底部时跟随新增文本。未保存思考的任务显示已有报告正文，本地规则检查直接展示检查报告。
