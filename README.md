...
5. 执行时会按检查项并发调用模型，并持续写入结果和进度；图片检查会按图片位置分批调用多模态模型，每批同时包含文档文本上下文和多张图片，用于检查图片内容、图片文字以及图文对应关系；单任务检查项并发数可在管理面调整。
...
“图片检查”页面会从文档中提取可识别图片并保存到 `instance/extracted_images/`，文件名包含图片在文档中的页码、段落、表格或工作表单元格等位置线索。系统同时提取文档文本，把文档上下文、图片清单和当前图片批次一起发送给支持图片输入的 OpenAI Chat Completions 兼容多模态模型。内置图片检查项包括图文对应、小语种文字、接线问题和画图规范，可在系统设置中修改提示词、新增扩展检查项、停用、删除和排序；提交任务时会保存所选检查项快照。
