...
- 任务执行：后台线程从 SQLite 队列拉取任务，默认全局并发 3、单用户并发 1、单任务检查项并发 1，可在管理面调整；文档文本会作为全文一次送入模型，图片检查会按提取位置逐张送入多模态模型。
...
5. 执行时会按检查项并发调用模型，并持续写入结果和进度；图片检查会按提取图片逐张调用多模态模型并按检查项汇总；单任务检查项并发数可在管理面调整。
...
“图片检查”页面会从文档中提取可识别图片并保存到 `instance/extracted_images/`，文件名包含图片在文档中的页码、段落、表格或工作表单元格等位置线索。内置图片检查项包括小语种文字、接线问题和画图规范，可在系统设置中修改提示词、新增扩展检查项、停用、删除和排序；提交任务时会保存所选检查项快照。图片检查需要选择支持图片输入的 OpenAI Chat Completions 兼容多模态模型。
