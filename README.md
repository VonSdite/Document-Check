...
3. 如果无法提取文本，系统会拒绝提交任务，并删除本次上传文件。单文档检查对中小文档使用全文模式，超过安全直接请求预算后自动切换为分段检查；多文档对照和跨语种检查的合并文本超过所选模型提供商上限时会拒绝提交。
4. 校验通过后，任务进入队列；后台调度器在 SQLite 事务中原子认领任务，按全局并发和单用户并发限制执行。运行任务会定期续租，进程异常退出后只有租约过期的任务会重新排队。
...
普通“单文档检查”任务会先在本地提取文本。中小文档以 `file: 文件名` 加全文内容的形式放入模型提示词；超过直接请求预算的长文档会按 PDF 页码、章节和段落边界分段，并保留少量相邻上下文和全文结构线索。系统不会把原始文件直接转发给模型。

单文档内置 AI 检查项按边界拆分为“文档规范性检查”“易理解性检查”“全文一致性检查”和“内容完整性检查”。规范性检查覆盖错别字、漏字、多字、标点、明确语法、术语书写、数字单位格式、客户表达、结构层级、编号引用、技术信息呈现和发布残留；原独立“错别字检查”在默认迁移中停用，避免重复报告。易理解性只处理内容已提供但难以理解的问题；全文一致性跨章节核对结构引用、名称术语、事实参数、条件步骤、结论和约束口径；完整性只处理能够由上下文证明为必需但尚未提供的信息。四项均优先使用章节路径定位，页码仅作为抽取文本中明确存在时的辅助线索。

所选模型提供商的“单次请求文本上限”用于计算单文档直接请求和分段大小。直接请求最多使用 12 万字符，单个长文档分段最多使用 4 万字符，并会根据提供商上限自动缩小；因此单文档总文本可以大于单次请求上限。“全文一致性检查”除分段内部检查外，还会从各分段提取参数、要求、条件、安全等级、术语定义和引用关系等事实，生成跨分段候选并再次交给模型核对。多文档对照和跨语种检查仍对合并后的全文执行提供商上限校验。
