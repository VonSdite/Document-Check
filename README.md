...
一个基于 Flask + SQLite 的文档智能门禁网站，支持用户上传 docx、pdf、txt、md、html、xlsx、xlsm、xls 文档和常见视频文件，并通过用户自行配置的 OpenAI Chat Completions 兼容模型执行单文档规范性、易理解性、内容正确性、内容完整性检查，以及多文档对照、跨语种文档一致性、多模态图片检查和视频抽帧检查。
...
单文档内置 AI 检查项按边界拆分为“文档规范性检查”“易理解性检查”“内容正确性检查”和“内容完整性检查”。规范性检查包含错别字、漏字、多字、标点、明确语法、术语书写、数字单位格式和客户表达检查；原独立“错别字检查”在默认迁移中停用，避免重复报告。易理解性只处理内容已提供但难以理解的问题；正确性只处理文内可验证的错误、矛盾和口径冲突；完整性只处理能够由上下文证明为必需但尚未提供的信息。四项均优先使用章节路径定位，页码仅作为抽取文本中明确存在时的辅助线索。
