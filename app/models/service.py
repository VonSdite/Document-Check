...
from app.models.client import normalize_reasoning_effort
...
def _load_user_model_providers(owner_subject: str) -> list[dict]:
    ...
    if not rows:
        return []
    configs_by_provider = {}
    for config in get_db().execute(
        """
        SELECT c.provider_id, c.model_name, c.force_disable_thinking, c.reasoning_effort
        FROM user_model_providers p
        JOIN user_model_configs c ON c.provider_id = p.id
        WHERE p.owner_subject = ?
        ORDER BY c.provider_id, c.sort_order, c.id
        """,
        (owner_subject,),
    ):
        configs_by_provider.setdefault(config["provider_id"], []).append(
            _model_config_from_row(config)
        )
    return [
        _provider_from_row(row, configs_by_provider.get(row["id"], [])) for row in rows
    ]
...
def _load_user_model_configs(provider_id: int) -> list[dict]:
    ...
    return [_model_config_from_row(row) for row in rows]


def _model_config_from_row(row) -> dict:
    return {
...
def get_enabled_models(owner_subject: str):
    models = []
...
def _find_enabled_model(model_id: str, owner_subject: str) -> dict | None:
    if ":" not in model_id:
        return None
    ...
    row = (
        get_db()
        .execute(
            "SELECT * FROM user_model_providers WHERE id = ? AND owner_subject = ? AND is_active = 1",
            (provider_id, owner_subject),
        )
        .fetchone()
    )
    if row is None or str(row["id"]) != provider_id:
        return None
    provider = _provider_from_row(row, _load_user_model_configs(row["id"]))
    for model_config in provider["models"]:
