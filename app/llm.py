...
5. 输入中的“PDF结构化表格”由文字坐标和矢量边框还原。HTML 的 rowspan、colspan 和 data-cell 表示合并范围；合并覆盖位置以及“[非文本图形或图标]”都不是数据缺失。只有明确标记为“[空单元格]”、表格置信度为 high，并且能够根据行标题、列标题或原文要求证明该位置必须填写时，才允许报告表格数据缺失；location 必须包含表格 ID 和单元格坐标。置信度为 medium 或 low 时不得报告数据缺失。
