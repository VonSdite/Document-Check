...
SENSITIVE_TERMS_INVALID_COLUMN = "不规范用语"
SENSITIVE_TERMS_STANDARD_COLUMN = "规范用语"
SENSITIVE_TERMS_CONTEXT_CHARS = 45
SENSITIVE_TERMS_LOCATIONS_PER_TERM = 5
...
def _find_sensitive_term_matches(document_text: str, rules: list[SensitiveTermRule]) -> list[dict]:
    ...
    document_index = _DocumentLocationIndex(text)
    matches = []
    for rule in rules:
        occurrences = list(_term_occurrences(text, rule.invalid))
        if not occurrences:
            continue
        samples = occurrences[:SENSITIVE_TERMS_LOCATIONS_PER_TERM]
        matches.append(
            {
                ...
                "excerpts": [
                    _excerpt_for(text, position, len(rule.invalid))
                    for position in samples
                ],
            }
        )
...
def _excerpt_for(text: str, position: int, length: int) -> str:
    start = max(0, position - SENSITIVE_TERMS_CONTEXT_CHARS)
    end = min(len(text), position + length + SENSITIVE_TERMS_CONTEXT_CHARS)
