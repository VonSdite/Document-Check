...
3. 标题只有编号没有标题文字，或只有标题文字没有“图/表+编号”，也视为标题不完整。
4. 如果图表标题在图片外部但已出现在提供的相邻文档文本中，不要判为缺失。
5. 对无法判断标题是否属于当前图片/表格、图片截取不完整或上下文不足的情况，标注“需人工确认”。

输出要求：
1. 先给出总体判断，说明是否发现图表标题缺失或不完整。
2. 按条列出问题：图片名称或位置、对象类型（图/表）、可见内容线索、缺失或不完整说明、建议补充的标题形式。
3. 对证据不足的问题单独标注“需人工确认”。
4. 如果未发现明显问题，明确说明“未发现明显图表标题规范问题”。不要编造图片或文档中不存在的标题。""",
        "sort_order": 35,
    },
    {
