...
3. 数字、单位和符号：日期、时间、数字、单位符号、全角/半角符号和同类书写形式的明确问题，例如日期格式混用、百分号或温度符号误用。不得以“10 MB”和“10MB”、“Type Ⅱ”和“TypeⅡ”等空格差异判定格式不规范；同一参数的数值不同属于内容正确性问题。
...
4. 不检查长句、指代不清、信息组织复杂等易理解性问题。
5. 不检查事实、参数、结论、名称含义或操作步骤的错误与前后冲突。
6. 不检查前提条件、术语解释、操作信息等内容是否缺失。
7. 不检查标题层级、章节顺序、目录、编号、交叉引用、字体、缩进、对齐、分页、页面布局、图片内容或链接有效性。
8. 不检查合规风险、商业承诺、敏感信息、安全提示、版本、版权、保密级别和修订记录。
...
重要能力边界：如果只提供一份待检查文档而没有产品规格、需求、接口定义、标准或其他权威素材，只能判断文内直接可证的冲突，不能凭外部常识、行业经验、公式推导或“通常应该如此”断言真实技术事实错误。只有系统同时提供权威依据时，才可以依据该材料判断外部事实正确性。

检查范围：
1. 名称与概念冲突：文中明确针对同一对象给出了互不相容的定义、含义或身份。近义称呼、全称与简称、类别与实例、产品族与具体型号以及未说明关系的不同名称，不足以证明冲突，不要报告。
...
判定方法：
1. 每条问题必须提供两处可以直接对照的证据；如果依据系统提供的权威素材判断，则提供待检查内容和权威依据各一处证据。只有一处原文、证据缺失或证据关系需要猜测时，不输出任何条目，也不要降级为 suggestion。
2. 两处证据必须描述同一对象、同一参数或同一命题，并且适用版本、场景、条件、状态和阶段相同。任一项不能确认时，直接忽略。
3. 在 location 中分别列出证据A、证据B的章节位置，在 excerpt 中分别逐字引用冲突原文。description 必须能用一句简洁的话说明：“证据A明确为X，证据B明确为Y；在相同条件C下，X和Y不能同时成立。”无法写出这句话时不要报告。
4. 单位换算后等价、数值实际相同、全称与简称并用、不同参数、不同部件、不同型号、不同条件或不同阶段均不构成冲突。两项内容相关、相似、数量相同或出现在同一表格中，也不能证明它们应当一致或一一对应。
5. PDF表格的行列关系可能在抽取时丢失。只有表头、行名、列名及单元格对应关系在文本中都明确时才能比较；无法确认某个数值对应哪个型号或字段时，直接忽略。
6. 不得使用“通常”“应该”“意味着”“由此推测”“可能存在”“理论上”等推理链构造冲突；不得利用文档未提供的专业公式或行业常识比较不同技术参数。
7. 输出前逐条反证：如果 description 中出现“一致”“等价”“不构成冲突”“可以合理解释”等结论，或先否定原候选再转而推导另一个问题，必须删除整个条目。summary 也不得保留已删除候选。
8. 不检查错别字、标点、书写格式和表达正式程度；不检查内容是否容易理解；不检查客户需要的信息是否缺失。
9. 不检查标题层级、目录、编号、图表视觉内容、页面布局和链接有效性。
