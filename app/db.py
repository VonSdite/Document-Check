...
DEFAULT_DOCUMENT_CHECK_ITEMS = (
    {
        "code": "compliance",
        "name": "文档规范性检查",
        "description": "检查语言文字、术语命名、日期数字与单位、结构层级、编号和交叉引用规范。",
        "prompt": """你是资深技术文档规范审查专家。请仅检查能够从抽取文本直接确认的语言文字、术语命名、日期数字与单位、结构层级、编号和交叉引用规范问题。

解析证据门槛：候选在去除抽取产生的空格与换行后仍然成立，并且能由连续原文、明确编号关系、目录与正文对应关系或边界清楚的字段内容核对。仅由空格位置、断词、分页或表格单元格拼接产生的差异不满足证据门槛。

检查范围：
1. 语言文字规范：明确的错别字、同音字或形近字误用、固定词语误写；能够根据上下文确定的漏字、多字；“的的”“进行进行”等机械性重复；修改方式基本唯一的成分残缺、搭配错误、关联词误用等语法错误。
2. 术语与命名书写：产品名、功能名、模块名、接口名、参数名、字段名、中英文名称、缩写和英文大小写的明确书写形式问题；文内定义、术语表、同一对象的稳定写法或系统提供的命名规范可作为判定依据。
3. 日期、数字、单位和标点符号：检查日期、时间、数字、单位符号、全角/半角符号、标点符号和成对标点的明确误用，以及同类内容在相同语境下的明显格式异常；“10 MB”和“10MB”、“Type Ⅱ”和“TypeⅡ”等仅有空格差异的候选不满足解析证据门槛；同一参数数值冲突归入全文一致性检查。
4. 结构与层级规范：依据明确的标题标记、编号层级或目录与正文对应关系，检查能够直接确认的标题缺失、层级跳跃、同级标题形式明显不一致、目录与正文标题明显不对应，以及标题与紧随其后的正文主题明显不匹配。
5. 编号与交叉引用规范：依据文内完整编号序列、引用原文和被引用对象，检查章节号、附录号、图号、表号、步骤号或公式号重复、跳号、错指、引用对象不存在，以及图题、表题或说明文字中直接可见的编号和命名问题。

本检查项准入条件：
1. issue 必须引用一处可独立核对的原文或一组明确的结构证据，并指出客观规范问题及最小必要修改；只有个人写作偏好或存在多种同等合理写法的候选不满足此条件。
2. suggestion 仅用于已有具体文本异常线索，但仍需核对一项明确的企业术语或命名规范才能定性的候选；必须写明现有线索和待核对的具体规范。没有具体异常线索、只基于个人偏好或仅能提出泛化改写建议的候选不进入报告。
3. 人名、地名、机构名、品牌名、产品名、型号、命令、代码、路径、URL和变量名，只有在文内定义、同一对象的稳定写法或系统提供的规范能够证明写法错误时才可判为 issue。
4. 结构、编号和引用候选必须能从标题标记、编号序列、目录与正文关系、引用原文和被引用对象中建立对应关系；只依赖字体、字号、缩进、对齐、页眉页脚、分页、图片视觉内容或链接实际可访问性的候选不进入报告。
5. 长句、指代和信息组织问题归入易理解性检查；事实、参数、结论或操作冲突归入全文一致性检查；必要信息缺失归入内容完整性检查。

问题类型建议使用：错别字、漏字或多字、机械性重复、明确语法错误、术语或命名书写不规范、日期数字单位或标点符号不规范、标题层级或章节结构不规范、编号或交叉引用不规范。"""
        + "\n\n"
...
解析证据门槛：候选必须来自能够独立阅读的完整自然语言句子、段落或边界明确的列表项。PDF中连续出现的表头、型号、字段、数值或单位序列，只有在行列对应关系可由抽取文本明确还原时才能作为证据。

检查范围：
1. 主体、对象和动作不明确：无法确定由谁执行、针对哪个设备/模块/页面/参数操作，或一句话中多个对象与后续动作的对应关系不清。
2. 指代不清：“其”“该”“上述”“前者”“后者”“相关”等存在多个可能指向对象；判定时列出至少两个能够由上下文支持的合理指向，或者指出缺失的先行对象。
3. 条件、范围和边界不清：“必要时”“适当”“部分场景”“相关版本”等缺少可理解的判断边界；多个条件之间的并且、或者、除非、仅当关系表达不清。
4. 句子结构复杂：一个句子混合过多条件、动作、例外、风险和结果，多层从句、插入语或多重否定使句子主干难以识别；判定依据是无法唯一还原条件、动作和结果的对应关系，而不是句子长度。
...
9. 语义重复和冗余：语法正确但相同含义反复表达，大量背景说明掩盖关键动作；安全、限制或关键操作的重复只有在未增加新条件且已经妨碍识别关键动作时才进入候选。

本检查项准入条件：
1. issue 必须引用完整证据，并具体指出客户无法唯一确定的主体、对象、指代、条件、动作顺序或信息关系；仅写“表达不清”“句子过长”或“建议优化”不构成判定。
2. suggestion 必须已有完整证据和具体理解障碍，但仍有一个可由人工核实的指代对象、适用范围或原始版式关系；description 中分别写明现有证据和待核实条件。
3. 表格、参数矩阵、规格清单和字段值序列只有在抽取文本保留了表头、行列和字段对应关系，且理解障碍不依赖丢失的页面版式时才进入候选；对应关系无法建立时不满足证据门槛。
4. 修改建议只拆分、重排或澄清原有信息，不增加条件、参数、步骤、结论或其他技术事实；无法保持原意时只提出需要澄清的对象。
5. 错别字、标点和书写格式归入文档规范性检查；信息完全缺失归入内容完整性检查；事实、参数、步骤或结论冲突归入全文一致性检查。

问题类型建议使用：主体不明确、操作对象不明确、指代不清、条件边界不清、含糊表达、句子结构复杂、多动作混杂、信息关系不清、专业表达难懂、重复冗余。"""
        + "\n\n"
        + _DOCUMENT_CHECK_OUTPUT_REQUIREMENTS
    ...
    },
    {
        "code": "consistency",
        "name": "全文一致性检查",
        "description": "检查全文内结构引用、名称术语、事实参数、条件步骤、结论和约束口径是否一致。",
        "prompt": """你是严谨的技术文档全文一致性审查专家。请跨章节、正文、表格、示例和说明文字进行对照，检查同一对象、同一事项或同一引用是否存在前后不一致、互相矛盾、引用错误或口径漂移。

证据来源门槛：一致性结论以文内可对照内容和系统同时提供的权威材料为依据。行业经验、外部公式和通常做法只能提示检索方向，不能作为报告证据。

检查范围：
1. 结构与内容一致性：目录、章节标题、小节范围、附录、图表和正文引用是否相互对应；章节标题与紧随其后的正文主题是否明显不符；章节号、图号、表号、步骤号、附录号、公式号是否存在重复、错指或对应对象不一致。
2. 名称与术语一致性：同一产品、部件、功能、页面、菜单、参数、字段、角色、机构、术语、缩写及中英文名称在不同位置使用不同叫法，并且差异会导致对象混淆或含义变化；文内定义、上下位关系以及全称与简称关系用于验证两处叫法是否等价。
3. 数据与技术参数一致性：同一参数、默认值、阈值、范围、端口、容量、数量、金额、版本、日期、环境要求或配置值在正文、表格、示例中是否不一致；比较前先完成单位换算并核对参数所属对象。
4. 条件与适用范围一致性：同一功能、步骤、限制或例外在不同位置的适用对象、型号、版本、场景、权限、状态、阶段或前提条件是否不一致。
5. 逻辑与结论一致性：前提与结论、原因与结果、正文与总结之间是否直接矛盾；同一状态是否被同时描述为支持和不支持、允许和禁止、成功和失败；步骤跳转、先后顺序或结果说明是否相互冲突。
6. 操作内容一致性：同一操作在不同位置的入口、对象、参数、动作、顺序、完成标志或预期结果是否互相冲突；候选必须包含两处操作描述或一处操作描述与一份权威依据。
7. 约束性与安全信息一致性：同一事项的“严禁、禁止、不得、必须、应、建议、可、允许、例外”等强制程度，以及风险后果、备份要求、权限要求、停机或重启要求是否前后矛盾。
8. 引用与对应关系一致性：正文声称“见图/表/章节/附件/下文”时，对应编号、标题或对象是否存在且匹配；清单数量、表头与数据项、步骤引用和配套物料的对应关系是否出现可直接确认的遗漏或错位。

本检查项准入条件：
1. issue 必须提供两处可直接对照的证据；依据权威材料判断时，分别提供待检查内容和权威依据。两处证据必须描述同一对象、同一参数、同一命题或明确的引用对应关系，并具有相同或重叠的型号、版本、场景、条件、状态和阶段。
2. location 分别列出证据A、证据B的位置，excerpt 逐字引用两处原文；description 使用“A为X、B为Y，在条件C下不能同时成立”的形式说明冲突关系。
3. suggestion 也必须具备两处直接证据并能确认比较对象相同，但允许型号、版本、场景、条件、状态或阶段中恰有一项无法从文内确认；description 必须写明待核实的具体条件。只有一处原文、没有对照对象或仅有泛泛怀疑的候选不满足准入条件。
4. 单位换算后等价、数值相同、全称与简称并用，或者两处证据属于不同参数、部件、型号、条件或阶段时，比较结果不构成冲突；issue 的冲突关系必须在不借助外部常识或未提供公式的情况下成立。
5. PDF表格候选必须能把每个比较值明确映射到表头、行名、列名、型号和字段；缺少任一关键映射时不满足表格证据门槛。
6. 章节、图表、步骤和附件引用可以使用“引用原文”和“被引用位置或全文结构线索”作为两处证据；全文结构线索必须足以确认对象确实缺失、错指或不对应。
7. 超链接格式、HTTP 状态、重定向、网络可达性、证书和访问权限由独立的“超链接有效性检查”规则处理。本检查不得报告链接失效、无法访问、权限不足、证书异常或重定向异常；只检查文内引用文字与对应对象之间能够由文档内容证明的一致性。
8. 错别字、标点、书写格式和表达正式程度归入文档规范性检查；理解障碍归入易理解性检查；必要信息缺失归入内容完整性检查。

问题类型建议使用：结构与内容不一致、名称或术语不一致、参数或数据不一致、版本或日期不一致、条件或适用范围不一致、逻辑或结论冲突、操作内容冲突、约束强度冲突、风险处置冲突、编号或交叉引用错误、清单或对应关系不一致。"""
        + "\n\n"
        + _DOCUMENT_CHECK_OUTPUT_REQUIREMENTS
        + "\n8. 如果未发现明确问题，summary 写“未发现明显全文一致性问题”，items 返回空数组。",
        "sort_order": 20,
    },
    {
        "code": "completeness",
...
必要性证据门槛：每个候选必须至少有一种明确触发依据，包括已有操作对前提、对象、参数或结果的依赖，风险动作对限制或恢复措施的依赖，文内明确引用或承诺，以及系统提供的模板或检查清单。通用写作经验和常见章节结构不能单独证明信息必需。

超链接证据规则：抽取文本会把链接显示为“显示文字（超链接：目标地址）”或“[超链接] …”。引用外部指南、手册或其他资料时，只要引用处或全文其他位置已经提供对应超链接、URL、文件路径或明确获取入口，就视为已经提供获取方式；不得再以“未提供该指南的获取方式或附录”“客户无法获取必要操作步骤”为由报告问题。超链接格式、HTTP 状态、重定向、网络可达性、证书和访问权限由独立的“超链接有效性检查”规则处理，本检查不得输出这些问题；也不得因为系统未读取链接目标内容，就推断客户无法获取资料。
...
8. 术语与字段说明：缩写、专业术语、参数或表格字段是理解和执行的关键，但首次出现或使用时没有必要解释；候选必须说明该术语或字段如何阻碍当前任务，行业通用词本身不构成必要性证据。
9. 表格与示例要素：从抽取文本能够确认表格或示例存在，并能明确还原字段与内容关系，但理解数据所必需的字段含义、单位、条件、占位值说明或示例结果缺失。
10. 未完成内容：TODO、TBD、XXX、待补充、此处插入、后续提供、见附件/下文但全文未提供相应内容等明确占位或未闭合引用；引用外部指南但全文既无超链接、URL、文件路径或明确获取入口，也未附带所需内容时，只有该指南确属完成当前任务所必需才能进入候选。

本检查项准入条件：
1. issue 必须引用必要性触发原文，并通过全文检索确认所需信息未在其他位置提供；description 按“已提供的任务或承诺—缺少的信息—对理解或执行的阻碍”说明因果关系。
2. suggestion 必须已有必要性触发原文，但文档类型、目标读者、适用范围或任务条件中恰有一项需要人工核实；description 写明现有触发证据和待核实条件。没有触发原文、只依据常见章节或泛称“建议补充”的候选不满足准入条件。
3. 修改建议只说明需要补充的信息类型，不自行补写参数值、操作步骤、业务规则或其他文档外事实。
4. 表格或示例候选必须能从抽取文本还原字段与内容关系；对应关系缺失时不满足证据门槛。
5. 信息已提供但难以理解归入易理解性检查；信息前后冲突归入全文一致性检查；错别字和格式问题归入文档规范性检查；标题层级、字体、页面布局、图片内容和链接实际可访问性不作为本检查的缺失依据。引用资料已有超链接或其他获取入口时，不得要求再把资料正文复制到当前文档或作为附录提供。
...
问题类型建议使用：适用范围缺失、前提条件缺失、环境或权限要求缺失、操作入口或对象缺失、关键步骤缺失、参数说明缺失、结果或验证方法缺失、异常或恢复说明缺失、限制或风险说明缺失、术语或字段解释缺失、占位或未完成内容。"""
    ...
    {
        "code": "hyperlink-validity",
        "name": "超链接有效性检查",
        "description": "通过本地规则和受控网络请求检查超链接格式、内部目标及 HTTP 可达性，不调用大模型。",
        "prompt": """本检查项由系统规则执行，不调用大模型。
系统会检查文档超链接是否包含目标地址、内部书签或页面是否存在、HTTP/HTTPS 链接是否可以到达，并受控处理重定向。
HTTP 404/410、目标缺失、不安全协议和明确不存在的内部目标报告为问题；登录或权限限制、超时、DNS、证书、限流、服务端故障及安全策略禁止访问的地址报告为需人工确认，不直接判定链接失效。
为防止服务端请求伪造，系统不会访问回环、私网、链路本地或保留地址；相同链接只检测一次并合并引用位置。""",
        "sort_order": 27,
    },
    {
        "code": "typo",
...
_COMPLIANCE_STOCK_PROMPT_MARKER_SETS = (
    ...
    (
        ...
        "不检查合规风险、商业承诺、敏感信息、安全提示、版本、版权",
    ),
    (
        "资深技术文档规范审查专家，熟悉面向客户资料的语言文字规范",
        "客户资料表达：内部沟通、研发评审、聊天式",
        "不检查标题层级、章节顺序、目录、编号、交叉引用",
    ),
    (
        "资深客户资料规范审查专家兼技术文档编辑",
...
_UNDERSTANDABILITY_STOCK_PROMPT_MARKER_SETS = (
    (
        "资深技术文档易理解性审查专家",
        "10. 表格文字难理解",
        "问题类型建议使用：主体不明确",
    ...
    (
        "本检查只审查能够识别为连续正文、完整操作句或边界明确的列表项",
...
_CONSISTENCY_STOCK_PROMPT_MARKER_SETS = (
    ...
    (
        "技术文档内容正确性审查专家",
        "每条明确问题原则上至少提供两处可以直接对照的文档证据",
        "证据不足但确有直接冲突线索时标为 suggestion",
    ),
    (
        "严谨的技术文档内容正确性审查专家",
        "只有一处原文、证据缺失或证据关系需要猜测时，不输出任何条目",
        "未发现明显内容正确性问题",
    ),
    (
        "严谨的技术文档全文一致性审查专家",
...
_COMPLETENESS_STOCK_PROMPT_MARKER_SETS = (
    (
...
def seed_defaults():
    ...
    _sync_understandability_prompt(db, now)
    _sync_completeness_prompt(db, now)
...
def _sync_understandability_prompt(db, updated_at: str):
    default_item = DEFAULT_CHECK_ITEMS_BY_CODE.get("understandability")
    if default_item is None:
        return
    row = db.execute(
        "SELECT name, description, prompt, sort_order FROM check_items WHERE code = 'understandability'"
    ).fetchone()
    if row is None:
        return
    prompt = str(row["prompt"] or "")
...
def _sync_completeness_prompt(db, updated_at: str):
    default_item = DEFAULT_CHECK_ITEMS_BY_CODE.get("completeness")
    if default_item is None:
        return
    row = db.execute(
        "SELECT name, description, prompt, sort_order FROM check_items WHERE code = 'completeness'"
    ).fetchone()
    if row is None:
        return
    prompt = str(row["prompt"] or "")
