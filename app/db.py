...
DEFAULT_DOCUMENT_CHECK_ITEMS = (
    {
        "code": "compliance",
        "name": "文档规范性检查",
        "description": "检查文字表达、客户语言、结构层级、编号引用、技术信息呈现和发布规范。",
        "prompt": """你是资深客户资料规范审查专家兼技术文档编辑。请检查能够从抽取文本直接确认，并会影响客户理解、资料可信度或正式交付质量的语言文字、结构组织、编号引用、技术信息呈现和发布规范问题。
...
2. 术语与命名书写：产品名、功能名、模块名、接口名、参数名、字段名、中英文名称、缩写和英文大小写的明确书写形式问题；文内定义、术语表、同一对象的稳定写法或系统提供的命名规范可作为判定依据；内部代号、研发代号或临时代称可由其上下文用途判定。
3. 数字、单位和符号：日期、时间、数字、单位符号、全角/半角符号和同类书写形式的明确问题，例如日期格式混用、百分号或温度符号误用；“10 MB”和“10MB”、“Type Ⅱ”和“TypeⅡ”等仅有空格差异的候选不满足解析证据门槛；同一参数数值冲突归入全文一致性检查。
4. 客户资料表达：结合完整句子或段落，检查内部沟通、研发评审、聊天式、情绪化、主观化、责备客户、推卸责任、夸大宣传、过度绝对化或明显过于随意的表达，例如“研发暂定”“客户自己处理”“随便配置即可”等未转换为正式客户语言的内容。
5. 结构与层级规范：依据明确的标题标记、编号序列或目录与正文对应关系，检查标题缺失、层级跳跃、同级标题形式明显不一致、章节或附录编号重复/断裂、目录与正文标题明显不对应、标题与紧随其后的正文主题明显不匹配。
6. 图表、步骤与交叉引用规范：依据文内完整编号序列和引用对象，检查图号、表号、步骤号、公式号、章节号、附录号重复、跳号、错指或引用对象不存在，以及图题、表题或说明文字中直接可见的编号和命名问题。
7. 技术信息与安全提示呈现：参数、单位、范围、默认值、版本、环境要求、限制条件、例外条件的标签或表达形式明显不完整、不规范或容易误认；注意、警告、危险、禁止、必须、建议等提示语的强制程度和呈现方式明显不当。本项只检查呈现规范，数值或要求前后冲突归入全文一致性检查，必要技术信息缺失归入内容完整性检查。
8. 发布与交付规范：TODO、TBD、XXX、“待补充”“此处插入”等占位符，研发备注、评审意见、草稿痕迹、内部责任人或不应面向客户暴露的内部信息；已经出现但明显未完成或相互错置的版本、修订、版权、保密级别和附件说明。

本检查项准入条件：
1. issue 必须引用一处可独立核对的原文或一组明确的结构证据，并指出客观规范问题及最小必要修改；只有个人写作偏好或存在多种同等合理写法的候选不满足此条件。
2. 人名、地名、机构名、品牌名、产品名、型号、命令、代码、路径、URL和变量名，只有在文内定义、同一对象的稳定写法或系统提供的规范能够证明写法错误时才可判为 issue；已有具体异常线索但需要企业命名规范核实时可判为 suggestion，并写明待核对的规范。
3. 结构、编号和引用候选必须能从标题标记、编号序列、引用原文和被引用对象中建立对应关系；只依赖字体、字号、缩进、对齐、页眉页脚、分页、图片视觉内容或链接实际可访问性的候选不满足本检查的文本证据门槛。
4. 法务、品牌、安全或企业规范类候选，只有原文存在明确风险线索时才进入报告；具备文内直接判定依据时为 issue，需要核对具体外部规范时为 suggestion。
5. 长句、指代和信息组织问题归入易理解性检查；事实、参数、结论或操作冲突归入全文一致性检查；必要信息缺失归入内容完整性检查。

问题类型建议使用：错别字、漏字或多字、机械性重复、标点误用、明确语法错误、术语或命名书写不规范、数字单位或符号格式不规范、客户表达不规范、标题层级或章节结构不规范、编号或交叉引用不规范、技术信息呈现不规范、安全提示呈现不规范、占位或内部残留、发布信息不规范。"""
        + "\n\n"
...
5. 信息已提供但难以理解归入易理解性检查；信息前后冲突归入全文一致性检查；错别字和格式问题归入文档规范性检查；标题层级、字体、页面布局、图片内容和链接实际可访问性不作为本检查的缺失依据。
