...
7. 错别字、标点、书写格式和表达正式程度归入文档规范性检查；理解障碍归入易理解性检查；必要信息缺失归入内容完整性检查。
...
超链接证据规则：抽取文本会把链接显示为“显示文字（超链接：目标地址）”或“[超链接] …”。引用外部指南、手册或其他资料时，只要引用处或全文其他位置已经提供对应超链接、URL、文件路径或明确获取入口，就视为已经提供获取方式；不得再以“未提供该指南的获取方式或附录”“客户无法获取必要操作步骤”为由报告问题。不要因为系统未打开链接、未读取链接目标内容或无法验证链接可访问性，就推断客户无法获取资料；链接实际失效或权限受限也不能仅凭抽取文本判定。
