...
用户和管理员各自的 `POST /tasks/<id>/retry-check` 接受 `code` 与 `execution`。重复或过期请求受执行编号和活动状态约束；当前模型列表不参与选择，已结束任务仅从原提供商补充访问密钥。详情的版本包含执行编号，重新执行时清理该项旧报告展示；模型输出保留先前请求并恢复增量读取。回归覆盖批量失败与取消混合重试、全部取消后重试、最新结果读取、重复提交、排队执行编号校验、快照模型请求及不可用结果、旧请求退出前重试、并发项隔离、失败重试、排队取消后重试、保留项追加执行、结果整理竞争、快照模型不可用、重复点击与归属校验。
