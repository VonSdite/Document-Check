...
用户与 console 的列表共用 `web/task_lists.py` 中的筛选条件构建器；用户条件包含当前身份与任务类型。`_task_filters.html` 和 `_task_refresh_controls.html` 提供共同控件，列表通过 `_partial=1` 请求获取统计与表格片段。片段中的操作回跳地址包含筛选参数，保持正常页面地址。
