...
`tasks/activity.py` 使用现有 `settings` 的任务专用键保存轻量运行状态。执行令牌在读写时校验，取消和完成共用短写事务。`tasks/selection.py` 统一按快照和重试范围解析本次执行的检查项；取消接口按相同范围校验待执行项。排队时的取消记录在认领事务内移交给新执行令牌，重试时清理上次记录。每项开始前在短事务中确认取消意图，已取消项直接生成取消结果。每个文档检查项拥有独立的取消事件，任务监测线程同时派发整任务与单项取消信号。已取消结果带有 `canceled` 标记，并保留原因；自动重试和“重试失败项”跳过主动取消项。
