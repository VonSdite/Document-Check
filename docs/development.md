...
`tasks/activity.py` 使用现有 `settings` 的任务专用键保存轻量运行状态。执行令牌在读写时校验，取消和完成共用短写事务。每个文档检查项拥有独立的取消事件，任务监测线程同时派发整任务与单项取消信号。已取消结果带有 `canceled` 标记，并保留原因；自动重试和“重试失败项”跳过主动取消项。

`web/task_activity.py` 负责状态展示和单项取消操作。详情页 GET 的 `_poll=1` 返回轻量阶段、进度与结果版本；版本相同时复用页面报告，变化时返回详情内容片段。`task-detail.js` 每 10 秒更新变化区域，保留展开状态、视口位置和视频节点；页面在后台、文本选中或报告控件编辑期间暂缓更新。

用户与管理员的 `POST /tasks/<id>/cancel-check` 路由沿用各自任务访问权限，正文包含检查项 `code`。仅文档、多文档对照和跨语种对照的活动模型请求支持单项取消；图片与视频保留合并请求。回归验证覆盖长思考后正文、纯思考异常重试、单项取消隔离、完成与取消竞争、轻量轮询及页面状态收尾。


`models/client.py` 的 `on_output` 回调发送 `start`、`thinking`、`content` 和 `end` 事件，每次 HTTP 尝试具有独立流标识。回调覆盖流式与服务返回的完整消息、异常、取消、重试和多媒体缺项补偿；报告正文仍使用 `on_content`。`tasks/model_output.py` 负责输出产物的缓冲写入和游标读取，`model-output.js` 仅在输出区展开时获取新增记录，以文本节点展示服务响应。

输出接口复用任务归属校验，仅读取状态所需字段；输出读取失败由页面自动重试，保存失败写入警告并继续报告处理。`tests/test_model_output.py` 验证不同模型名称及自定义别名下的通用展示、多种思考字段、流式分片与完整消息、纯正文响应，以及并发追加、Unicode 增量边界、重试与取消尾部、文件清理和写入异常。路由测试覆盖归属隔离、游标验证、缓存策略和顶部折叠区；模型测试覆盖 64,000 段上限及思考与正文分离。

## 任务列表交互

用户与 console 的列表共用 `web/task_lists.py` 中的筛选条件构建器；用户条件包含当前身份与任务类型。`_task_filters.html` 和 `_task_refresh_controls.html` 提供共同控件，列表通过 `_partial=1` 请求获取统计与表格片段。片段中的操作回跳地址包含筛选参数，保持正常页面地址。关键词采用字面包含匹配，对照任务仅从元数据的 `groups[].files[].original_filename` 读取文件名；元数据解析支持 Unicode 转义，异常元数据按空对象处理。筛选区显示搜索范围，并提示尚未应用的条件。
