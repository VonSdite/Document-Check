...
用户与管理员的 `POST /tasks/<id>/cancel-check` 路由沿用各自任务访问权限，正文包含检查项 `code`。仅文档、多文档对照和跨语种对照的活动模型请求支持单项取消；图片与视频保留合并请求。回归验证覆盖长思考后正文、GLM 5.3 重试、单项取消隔离、完成与取消竞争、轻量轮询及页面状态收尾。
...
输出接口复用任务归属校验，仅读取状态所需字段；输出读取失败由页面自动重试，保存失败写入警告并继续报告处理。`tests/test_model_output.py` 验证并发追加、Unicode 增量边界、重试与取消尾部、文件清理和写入异常。路由测试覆盖归属隔离、游标验证、缓存策略和顶部折叠区；模型测试覆盖 64,000 段上限及思考与正文分离。
