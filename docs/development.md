...
`tasks/activity.py` 使用现有 `settings` 的任务专用键保存轻量运行状态。执行令牌在读写时校验，取消和完成共用短写事务。`tasks/selection.py` 统一按快照和重试范围解析本次执行的检查项；取消接口按相同范围校验待执行项。排队时的取消记录在认领事务内移交给新执行令牌，重试时清理上次记录。取消与每项开始共用短事务，待执行项立即进入已取消状态，正在执行的项进入取消中。调度循环每秒通过 Future.cancel() 移除尚未开始的已取消项并保存结果；退出队列后即可接收同项重试，执行编号继续递增。每个文档检查项拥有独立的取消事件，任务监测线程同时派发整任务与单项取消信号。已取消结果带有 `canceled` 标记，并保留原因；模型自动重试遵守取消信号。用户通过“重试未完成项”批量重新执行失败或已取消的项，也可在标题旁重试单项。

`web/task_activity.py` 负责状态展示和单项取消操作。详情页 GET 的 `_poll=1` 返回轻量阶段、进度与结果版本；版本相同时复用页面报告，变化时返回详情内容片段。取消状态参与版本判断，展示层根据活动记录即时补齐取消结果；操作响应直接更新该项状态，迟到的轻量状态响应按请求版本丢弃。`task-detail.js` 每 10 秒更新变化区域，保留展开状态、视口位置和视频节点；页面在后台、文本选中、报告控件编辑或取消确认气泡打开期间暂缓更新。检查项标题旁显示“取消执行”和带“状态：”前缀的阶段文字；结束后隐藏阶段文字。模型输出折叠标题仅显示名称，展开后按思考和正文增量显示内容。

用户与管理员的 `POST /tasks/<id>/cancel-check` 路由沿用各自任务访问权限，正文包含检查项 `code`。文档、多文档对照和跨语种对照从排队到检查项执行结束前支持单项取消；图片与视频保留合并请求。回归验证覆盖长思考后正文、纯思考异常重试、排队取消的认领移交、预处理与线程池等待阶段取消、前项阻塞时取消结果落盘及重试入队、单项取消隔离、重试范围校验、完成与取消竞争、轻量轮询及页面状态收尾。
...
用户与 console 的列表共用 `web/task_lists.py` 中的筛选条件构建器；用户条件包含当前身份与任务类型。`_task_filters.html` 和 `_task_refresh_controls.html` 提供共同控件，列表通过 `_partial=1` 请求获取统计与表格片段。片段中的操作回跳地址包含筛选参数，保持正常页面地址。关键词采用字面包含匹配；普通用户只匹配文档名称，console 还匹配用户、账号和 IP。对照任务仅从元数据的 `groups[].files[].original_filename` 读取文件名；元数据解析支持 Unicode 转义，异常元数据按空对象处理。筛选通过提交表单生效，刷新图标在请求期间旋转，错误信息在请求失败时显示。
