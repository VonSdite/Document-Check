...
`tasks/activity.py` 使用现有 `settings` 的任务专用键保存轻量运行状态。执行令牌在读写时校验，取消和完成共用短写事务。`tasks/selection.py` 统一按快照和重试范围解析本次执行的检查项；取消接口按相同范围校验待执行项。排队时的取消记录在认领事务内移交给新执行令牌，重试时清理上次记录。每项开始前在短事务中确认取消意图，已取消项直接生成取消结果。每个文档检查项拥有独立的取消事件，任务监测线程同时派发整任务与单项取消信号。已取消结果带有 `canceled` 标记，并保留原因；模型自动重试和任务级“重试失败项”跳过主动取消项。用户通过单项“重试”可重新执行已取消或失败的项。
...
`tasks/retries.py` 在短事务中接收单项手动重试，校验任务快照、检查项状态和执行编号。运行中的任务将意图保存在活动记录中；任务线程池等旧 Future 退出后清理该项结果并重新提交。每次执行拥有独立取消事件，监测线程按执行编号匹配信号；新请求沿用任务的模型快照。线程池接收重试和进入结果整理在活动记录事务中互斥。任务结束后，单项重试设置原有 `retry_check_codes_json` 并进入队列；其他结果与标注继续保留。

用户和管理员各自的 `POST /tasks/<id>/retry-check` 接受 `code` 与 `execution`。重复或过期请求受执行编号和活动状态约束；当前模型列表不参与选择，已结束任务仅从原提供商补充访问密钥。详情的版本包含执行编号，重新执行时清理该项旧报告展示；模型输出保留先前请求并恢复增量读取。回归覆盖旧请求退出前重试、并发项隔离、失败重试、排队取消后重试、保留项追加执行、结果整理竞争、快照模型不可用、重复点击与归属校验。
