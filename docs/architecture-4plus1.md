...
1. Web worker 接收上传、校验文件和模型配置。
   用户端和管理端共用检查项选择组件：单文档、跨语种、图片和视频检查默认全选，多文档对照检查默认选择第一项，提交前校验至少选择一项。
...
app/                              Python 命名空间包
  ...
  tasks/
    ...
    selection.py                  检查项快照解析与本次重试范围
    retries.py                    批量与单项手动重试校验及排队
...
任务执行状态使用 `settings` 中的 `task_activity:<task_id>` 键保存紧凑 JSON，包含执行令牌、任务阶段和检查项阶段。此记录只承载临时状态，不保存正文、思考内容或凭据。读写按主键定位，状态变化时写入，取消、检查项开始与完成在短事务内串行判定。排队阶段也可写入取消意图，监督器在认领事务内将记录绑定到新执行令牌；重试时清理上次执行记录。任务正常收尾、监督器回收或删除时清理记录；执行令牌隔离不同运行，读取只接受当前活动任务的记录。

任务取消监测线程每秒读取一次整任务和检查项信号。文档、多文档对照及跨语种检查为每项维护独立取消事件，用户通过标题旁的“取消执行”及确认气泡提交取消；待执行项在取消事务中直接进入已取消状态，页面立即展示取消结果和重试按钮；线程池每秒清理已取消的排队 Future 并保存结果，无须等待其他检查项结束。取消与检查项开始共用短事务，已经开始的项先进入取消中；LLM 响应收到取消后关闭连接并停止重试，本地检查返回时确认取消；其他检查项保留执行。标题状态带“状态：”前缀，结束后隐藏。图片、视频保持合并请求，仅提供整任务取消。已取消或失败的检查项提供“重试”：文档任务复用当前线程池，旧执行退出后才重新提交；已结束的任务通过调度队列执行单项，图片和视频也在结束后重试单项。任务列表的“重试”和详情页的“重试未完成项”批量执行失败及主动取消项，全部取消的任务同样支持；成功结果与标注保留，提前中断任务在原执行范围内补跑无结果项。批量选择与入队在同一短事务中按主键读取最新状态，排队时初始化重试项的递增编号。每次执行使用独立编号和取消事件，模型参数与检查提示取自任务快照；已结束任务从原提供商补充访问密钥，原模型不可用时记录失败。线程池在短事务中接收重试或进入结果整理，保证已接收的重试获得执行；其余结果与标注继续保留。

LLM 适配层通过 `on_activity` 回调报告等待、思考、输出与重试阶段。模型请求常规失败时最多自动重试两次，分别在失败后等待 2 秒和 4 秒；等待期间支持取消。`on_output` 独立传递模型返回的思考、正文和请求边界，供输出记录模块追加保存。展示逻辑以 Chat Completions 响应字段为依据，所有模型及自定义别名共用同一采集、保存和增量展示流程；输出展示与请求重试策略分别处理。开启思考时的纯思考上限为 64,000 段或 128,000 字符，关闭思考时为 64 段或 256 字符。请求超时约束连接和读取等待，任务没有统一的总执行时限；90 秒租约用于执行权与失联恢复。
...
管理员与用户的 `GET /tasks/<id>/model-output?cursor=<字节位置>` 接口沿用任务访问权限，按主键检查归属和状态，每次从游标读取至多 64 KiB 的完整记录。该短请求独立于 10 秒报告刷新，展开时约每秒读取增量，追赶已有记录时连续分页；活动任务折叠时通过 `state_only=1` 仅读取轻量检查状态，后台标签页暂停同步。浏览器为每项保留最新请求的思考和正文，以文本节点追加内容，并保留展开状态与页面位置。手动重试响应后立即清空该项旧错误和输出，自动重试随轻量状态同步清理；事件中的检查项执行编号隔离旧请求迟到的输出，报告继续每 10 秒更新。

### 任务列表筛选与局部更新

`web/task_lists.py` 为用户界面和 console 复用状态、标注状态与关键词筛选。用户查询始终以 `owner_subject` 和任务类型限制范围；分页计数与记录查询使用相同条件。普通用户的关键词只在文件名中按文字包含匹配，console 同时匹配用户信息；对照任务通过 SQLite JSON 函数读取 `groups[].files[].original_filename`，候选范围由任务归属与类型限定。标注筛选查询统计缓存，请求线程最多准备当前用户最近 20 条记录，历史统计由监督器维护。

列表 GET 的 `_partial=1` 仅渲染统计与任务列表片段，并省略新建任务区域及其模型、检查项查询。`task-list.js` 处理两端的筛选、分页、前进后退、手动刷新与 10 秒自动刷新；按请求版本丢弃过期响应。页面复用表头和未变化的任务行，维护 URL、列宽、勾选、横向滚动及页面视口。共享筛选与刷新模板保持五类任务界面一致。
