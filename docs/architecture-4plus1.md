...
任务取消监测线程每秒读取一次整任务和检查项信号。文档、多文档对照及跨语种检查为每项维护独立取消事件，用户通过标题旁的“取消执行”及确认气泡提交取消；待执行项直接跳过，LLM 响应收到取消后关闭连接并停止重试，本地检查返回时确认取消；其他检查项保留执行。标题状态带“状态：”前缀，结束后隐藏。图片、视频保持合并请求，仅提供整任务取消。
