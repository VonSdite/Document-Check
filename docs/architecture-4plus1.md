...
`web/task_lists.py` 为用户界面和 console 复用状态、标注状态与关键词筛选。用户查询始终以 `owner_subject` 和任务类型限制范围；分页计数与记录查询使用相同条件。关键词在文件名与用户信息中按文字包含匹配；对照任务通过 SQLite JSON 函数读取 `groups[].files[].original_filename`，候选范围由任务归属与类型限定。标注筛选查询统计缓存，请求线程最多准备当前用户最近 20 条记录，历史统计由监督器维护。
