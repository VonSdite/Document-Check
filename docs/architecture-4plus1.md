...
任务取消监测线程每秒读取一次整任务和检查项信号。文档、多文档对照及跨语种检查为每项维护独立取消事件，用户通过标题旁的“取消执行”及确认气泡提交取消；待执行项直接跳过，LLM 响应收到取消后关闭连接并停止重试，本地检查返回时确认取消；其他检查项保留执行。标题状态带“状态：”前缀，结束后隐藏。图片、视频保持合并请求，仅提供整任务取消。已取消或失败的检查项提供“重试”：文档任务复用当前线程池，旧执行退出后才重新提交；已结束的任务通过调度队列执行单项，图片和视频也在结束后重试单项。每次执行使用独立编号和取消事件，模型参数与检查提示取自任务快照。线程池在短事务中接收重试或进入结果整理，保证已接收的重试获得执行；其余结果与标注继续保留。
