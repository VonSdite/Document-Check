...
LLM 适配层通过 `on_activity` 回调报告等待、思考、输出与重试阶段。`on_output` 独立传递模型返回的思考、正文和请求边界，供输出记录模块追加保存。开启思考时的纯思考上限为 64,000 段或 128,000 字符，关闭思考时为 64 段或 256 字符。请求超时约束连接和读取等待，任务没有统一的总执行时限；90 秒租约用于执行权与失联恢复。
